...
//						t.printStackTrace();//ignore
	...
	// in memory player
	public JavaSoundPlayer(byte[] data, AudioFormat format) {
		...
		try {
		    ...
		    playerClip = (Clip) AudioSystem.getLine(info);
		    //audioInStream = AudioSystem.getAudioInputStream(is);
		    audioInStream = new AudioInputStream(is, format, data.length / format.getChannels());
		    playerClip.open(audioInStream);
...
//			}
			...
			if (LOG.isLoggable(Level.WARNING)) {
				LOG.warning("Cannot create sound player, IO error: " + t.getMessage());
			}
		}
	}
